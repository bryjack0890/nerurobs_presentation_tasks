scenario = "Second-order Rule trial"; #Name, used for references 
...
array {
   ...
   $k = '$i + 1';
   bitmap { filename = "2ndRInstructionCues\\InstructionCue$k.png"; description = "$k"; } "ic$k"; 
   ENDLOOP;
...
array {
   LOOP $r 25; #How many cues
   $m = '$r + 1';
   bitmap { filename = "ResponseCues\\ResponseCue$m.png"; description = "$m"; } "rc$m"; 
   ENDLOOP;
} responsecues_array;
#########-END Response Cues-#########
...
string filename = logfile.subject() + "2ndR PrintOut";
...
int rpt = 4000; #Timing for the Response Period in miliseconds
int rctt = rct + gt + fbt; #Total timing for cues and feedback
#########-END Declared Timings-#########
...
array<int> correct_array[25][4] = { { 1,2,3,4 }, { 1,4,2,3 }, { 1,4,3,2 }, { 1,2,4,3 }, { 2,1,4,3 }, { 4,1,2,3 }, { 4,1,3,2 }, { 2,1,3,4 }, { 4,3,1,2 }, { 4,2,1,3 }, { 3,4,1,2 }, { 3,2,1,4 }, { 3,2,4,1 }, { 2,3,4,1 }, { 2,4,3,1 }, { 3,4,2,1 }, { 4,3,2,1 }, { 2,4,3,1 }, { 4,2,3,1 }, { 1,3,2,4 }, { 1,3,4,2 }, { 3,1,2,4 }, { 3,1,4,2 }, { 2,4,1,3 }, { 2,3,1,4 } };
...
begin
	...
	int x = random(100,3400);
	int y = ipt - ict - x;
	int a = random(100,2400);
	int b = rpt - rct - gt - fbt - a;
...
#Logic for response cues.
	...
	stim_event6.set_target_button(correct_array[rca][ica]);
...
#Logic for feedback. 
   ...
   if last.reaction_time() >= rct || last.reaction_time() <= 0 then
		...
		if last.button() == correct_array[rca][ica] then
...
output.print(correct_array[rca][ica]);
output.print("\n");
